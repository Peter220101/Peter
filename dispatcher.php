...
$enableGoogleMaps = (bool) ($config['enable_google_maps'] ?? false);
...
function renderMapMarkers() {
    ...
    if (!window.google || !google.maps || !google.maps.marker || !google.maps.marker.AdvancedMarkerElement) return;
...
function bootstrapUi() {
    ...
    if (!<?php echo $enableGoogleMaps ? 'true' : 'false'; ?>) {
        showMapFallback('Google Maps desactivado por configuración (ENABLE_GOOGLE_MAPS=false).');
    }
}
...
<?php if ($enableGoogleMaps && $googleMapsApiKey !== 'YOUR_GOOGLE_MAPS_API_KEY'): ?>
