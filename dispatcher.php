...
<div class="app-shell">
    ...
    <aside class="leftbar" aria-label="Navegación principal">
        <div class="nav-icon active" title="Buscar">🔎</div><div class="nav-icon" title="Servicios">📋</div><div class="nav-icon" title="Operadores">👥</div><div class="nav-icon" title="Alertas">🔔</div><div class="nav-icon" title="Mensajes">💬<span class="nav-badge">17</span></div><div class="nav-icon" title="Zonas">🗺️<span class="nav-badge">30</span></div><div class="nav-icon" title="Configuración">⚙️</div><div class="nav-icon" title="Usuarios">🧑</div><div class="nav-icon" title="Facturación">🧾</div>
    </aside>
...
function bindUiEvents() {
    document.getElementById('zoomInBtn').addEventListener('click', () => appState.map.setZoom(appState.map.getZoom() + 1));
    document.getElementById('zoomOutBtn').addEventListener('click', () => appState.map.setZoom(appState.map.getZoom() - 1));
    document.getElementById('recenterBtn').addEventListener('click', () => { appState.map.setCenter({ lat: -2.170998, lng: -79.922359 }); appState.map.setZoom(12.6); });
    document.getElementById('toggleTrafficBtn').addEventListener('click', () => {
        appState.trafficEnabled = !appState.trafficEnabled;
        appState.trafficLayer.setMap(appState.trafficEnabled ? appState.map : null);
    });
}
...
async function initGoogleDispatchMap() {
    ...
    renderStats(); renderServiceList(); renderQueueBoard(); renderDriversTable(); renderMapMarkers(); bindUiEvents();
...
<script async src="https://maps.googleapis.com/maps/api/js?key=<?php echo htmlspecialchars($googleMapsApiKey, ENT_QUOTES, 'UTF-8'); ?>&loading=async&libraries=marker&callback=initGoogleDispatchMap"></script>
</body>
</html>
